...
use crate::state::{LikerRecord, PostState};
...
// examine the content of frontend,and return the pointed LikePostAccount
impl<'a> LikePostAccount<'a> {
...
// ==========================================
pub struct LikePost<'a> {
    pub accounts: LikePostAccount<'a>,
    bump: u8,
}
...
impl<'a> LikePost<'a> {
    pub fn try_from_parts(
        ...
        let parsed_accounts = LikePostAccount::try_from_bytes(accounts)?;
        ...
        let (expected_pda, bump) = Address::find_program_address(
            &[
                b"like",
                parsed_accounts.post_state.address().as_ref(),
    ...
    pub fn process(&mut self, program_id: &Address) -> ProgramResult {
        ...
        let like_seeds = [
            Seed::from(b"like"),
            Seed::from(accounts.post_state.address().as_ref()),
        ...
        like_record.target = accounts.post_state.address().clone();
        ...
        let mut post_data = accounts.post_state.try_borrow_mut()?;
        let post_state = PostState::load_mut(&mut post_data)?;

        //MATH SAFETY :Use 'check_add'to prevent u32 arithmetic over vulnerabilities.
        post_state.likes = post_state.likes.checked_add(1).unwrap_or(post_state.likes);
        drop(post_data);
        Ok(())
    }
}

pub fn likepost(data: &[u8], accounts: &[AccountView], program_id: &Address) -> ProgramResult {
    let mut like_ix = LikePost::try_from_parts(data, accounts, program_id)?;
